...
def get_pbc_distances(
    ...
    # correct for pbc
    offsets = torch.bmm(
        lattice.repeat_interleave(num_bonds, dim=0), to_jimages.unsqueeze(2)
    ).squeeze(2)
...
def radius_graph_pbc(
    ...
    pbc_offsets = torch.bmm(lattice, unit_cell_batch)
...
def min_distance_sqr_pbc(cart_coords1, cart_coords2, lattice, num_atoms, device):
